...
},{}],"swaggerXML/swaggerXml.ts":[function(require,module,exports) {
"use strict";
...
var swaggerData = __importStar(require("./swagger.json"));

var headStyle = "swimlane;childLayout=stackLayout;horizontal=1;startSize=30;horizontalStack=0;resizeParent=1;resizeParentMax=0;resizeLast=0;collapsible=1;marginBottom=0;strokeColor=#9673a6;fillColor=#e1d5e7;fontColor=#660066;";
var itemStyle = "text;dashed=1;strokeColor=#A680B8;fontColor=#634C6E;spacingLeft=10;";
var itemHeight = 30;
var itemWidth = 250;

var getXML = function getXML() {
  // @ts-ignore
  var keys = Object.entries(swaggerData.components.schemas);
  var data = keys.map(function (item, index) {
    return {
      type: item[1].hasOwnProperty('enum') ? 'enum' : 'DTO',
      name: item[0],
      props: item[1].properties || item[1].enum,
      desc: item[1].description,
      line: Math.ceil((index + 1) / 10)
    };
  });
  var DtoSrc = data.filter(function (item) {
    return item.type === 'DTO';
  }).filter(function (item) {
    return item.name.includes('taff');
  });
  var EnumSrc = data.filter(function (item) {
    return item.type === 'enum';
  });
  var DTOdata = DtoSrc.map(function (item, index) {
    return __assign(__assign({}, item), {
      line: Math.ceil((index + 1) / 10)
    });
  });
  var EnumData = EnumSrc.map(function (item, index) {
    return __assign(__assign({}, item), {
      line: Math.ceil((index + 1) / 10)
    });
  });

  var getChunk = function getChunk(arr, size) {
  ...
  var chunkDTO = getChunk(DTOdata, 10);

  var getXmlDTO = function getXmlDTO(arr) {
    return arr.map(function (dto, indexDTO) {
      var itemID = indexDTO + 1 + "-line" + dto.line + "-item";
      var index = indexDTO + 1;
      var itemX = itemWidth * index + 30 * index;
      var itemY = dto.line * 400;
      var container = "<mxCell id=\"" + itemID + "\" value=\"" + dto.name + "\" style=\"" + headStyle + "\" vertex=\"1\" parent=\"1\" >";
      var geoHeader = "<mxGeometry x=\"" + itemX + "\" y=\"" + itemY + "\" width=\"" + itemWidth + "\" height=\"" + itemHeight * 3 + "\" as=\"geometry\">" + ("<mxRectangle x=\"" + itemX + "\" y=\"" + itemY + "\" width=\"" + itemWidth + "\" height=\"" + itemHeight + "\" as=\"alternateBounds\"/>") + "</mxGeometry>";
      var end = "</mxCell>";
      var head = container + geoHeader + end; // @ts-ignore

      var blocks = Object.entries(dto.props).map(function (prop, indexProp) {
        var name = prop[0];
        var type = prop[1].type;
        var itemY = indexProp + 30;
        var block = "<mxCell id=\"" + itemID + "-" + indexProp + "-prop\" value=\"" + name + ": " + type + "\" style=\"" + itemStyle + "\" vertex=\"1\" parent=\"" + itemID + "\">";
        var geoBlock = "<mxGeometry y=\"" + itemY + "\" width=\"" + itemWidth + "\" height=\"" + itemHeight + "\" as=\"geometry\"/>";
        var end = "</mxCell>";
        return block + geoBlock + end;
      });
      var props = blocks.reduce(function (acc, block) {
        return acc + block;
      }, '');
      return head + props;
    });
  ...
  var getXmlLine = function getXmlLine(arr) {
    return arr.reduce(function (acc, dto) {
      return acc + dto;
    }, '');
  };

  var test = chunkDTO.map(function (item) {
    var xml = getXmlDTO(item);
    return getXmlLine(xml);
  });
  console.log(getXmlLine(test));
};

getXML();
},{"./swagger.json":"swaggerXML/swagger.json"}],"../../AppData/Roaming/nvm/v14.17.6/node_modules/parcel-bundler/src/builtins/hmr-runtime.js":[function(require,module,exports) {
...
},{}]},{},["../../AppData/Roaming/nvm/v14.17.6/node_modules/parcel-bundler/src/builtins/hmr-runtime.js","swaggerXML/swaggerXml.ts"], null)
//# sourceMappingURL=/swaggerXml.5188d578.js.map